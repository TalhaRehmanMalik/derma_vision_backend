...
from database import get_db
from models import Scan, Dermatologist  # <-- Dermatologist model yahan add kiya
from utils.auth import get_current_user_id
from utils.inference import predict, save_image, allowed_file
from utils.logger import get_logger
...
async def predict_image(
    ...
    # Active dermatologists fetch karne ka logic (Without Email)
    doctors = db.query(Dermatologist).filter(Dermatologist.is_active == True).all()
    ...
    return {
        ...
        "inconclusive":             result["inconclusive"],
        "disclaimer":               disclaimer,
        "recommended_dermatologists": doctors_list,  # <-- Frontend ke liye doctors list
    }
